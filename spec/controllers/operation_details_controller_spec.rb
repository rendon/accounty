  ...
  context 'when the user is logged in' do
    describe 'POST #create' do
      before(:each) do
        ...
        account_attributes = attributes_for(:account, name: 'Cash')
        @account = @company.accounts.create(account_attributes)
      ...
      context 'valid operation detail' do
        it 'increments OperationDetails by 1' do
          expect do
            post :create, user_id: @user.id, company_id: @company.id,
                          ...
                          operation_detail: { account_id: @account.id,
                                              amount: 100.0 }
        ...
        it 'redirects to the operation page' do
          post :create, user_id: @user.id, company_id: @company.id,
                        ...
                        operation_detail: { account_id: @account.id,
                                            amount: 100.0 }
          path = user_company_operation_path(@user, @company, @operation)
